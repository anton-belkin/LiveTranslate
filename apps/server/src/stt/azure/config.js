...
export function loadAzureSpeechConfig(env = process.env) {
    ...
    if (!key || !region) {
        throw new Error("Missing Azure Speech config. Set AZURE_SPEECH_KEY and AZURE_SPEECH_REGION.");
    }
    const autoDetectLanguages = withFallback(splitLangList(autoDetectRaw ?? "de-DE,en-US"), ["de-DE", "en-US"]);
    const translationTargets = withFallback(splitLangList(translationTargetsRaw ?? "de,en"), ["de", "en"]);
    // #region agent log
    debugLog({
