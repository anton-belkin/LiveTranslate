export async function groqTranslate(config, input) {
    const payload = {
    ...
    const systemPrompt = "You are a translation engine. Input text comes from STT and may be partial or noisy. " +
...
function normalizeSourceLang(value, targetLangs) {
    ...
    if (targetLangs.includes(normalized))
        return normalized;
    return undefined;
}
